  ...
  return (
    <View style={{
      ...
      {NAV.map(item => {
        if (item.id === 'ai') {
          return (
            <TouchableOpacity key="ai" onPress={() => onSelect('ai')} activeOpacity={0.8}
              style={{ marginTop: -14 }}>
              <LinearGradient
                ...
                <Text style={{ color: active === 'ai' ? '#07071a' : C.blue, fontSize: 20, fontWeight: '900' }}>✦</Text>
              </LinearGradient>
            </TouchableOpacity>
          );
        }
        ...
        return (
          <TouchableOpacity key={item.id} onPress={() => onSelect(item.id)} activeOpacity={0.7}
            ...
            <Text style={{ fontSize: 9, letterSpacing: 1, color: on ? C.blue : C.muted, fontWeight: on ? '700' : '400' }}>
              {item.label}
            </Text>
          </TouchableOpacity>
        );
